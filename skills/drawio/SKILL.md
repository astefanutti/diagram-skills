...
**NEVER read image files (PNG, SVG, PDF, JPG) directly in the main context.** Images accumulate across iterations and diagram sessions, fill the context window, and corrupt after compaction — causing `API Error: 400 "Could not process image"` that breaks the conversation.

If you need to visually verify an exported diagram, spawn a **sub-agent** via the Agent tool:

```
Agent({
  description: "Verify diagram export",
  prompt: "Read the image at <path> using the Read tool. Check that the diagram rendered correctly: nodes are visible, edges connect properly, labels are readable, no blank/corrupt output. Report findings as text only — do NOT output the image."
})
```

The sub-agent's context is isolated — image data never enters the main conversation.
